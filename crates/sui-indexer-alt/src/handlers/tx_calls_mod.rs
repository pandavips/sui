...
use crate::{db, models::transactions::StoredTxCallsMod, schema::tx_calls_mod};
...
impl Handler for TxCallsMod {
    const NAME: &'static str = "tx_calls_mod";

    const BATCH_SIZE: usize = 100;
    const CHUNK_SIZE: usize = 1000;
    const MAX_PENDING_SIZE: usize = 10000;

    type Value = StoredTxCallsMod;
    ...
    fn process(checkpoint: &Arc<CheckpointData>) -> Result<Vec<Self::Value>> {
        ...
        for (i, tx) in transactions.iter().enumerate() {
            ...
            values.extend(move_calls.iter().map(|(package, module, _)| StoredTxCallsMod {
                tx_sequence_number,
    ...
    async fn commit(values: &[Self::Value], conn: &mut db::Connection<'_>) -> Result<usize> {
        Ok(diesel::insert_into(tx_calls_mod::table)
            .values(values)
